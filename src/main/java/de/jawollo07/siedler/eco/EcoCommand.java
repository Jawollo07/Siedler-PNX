...
public class EcoCommand extends Command {
    ...
    protected void buildCommandTree(RouteTree tree) {
        tree.getRoot().then(RouteNode.literal("help").exec(context -> CommandResult.success()));
        ...
        tree.getRoot().then(RouteNode.literal("show")
            .then(RouteNode.argument("team", new StringNode()).exec(context -> {
                try {
                    ...
                    String ecoSymbol = ecoManager.getCurrency("s");
                    context.getSender().sendMessage(prefix + "Der Kontostand von: " + teamName + " ist: " + balance + ecoSymbol);
                    return CommandResult.success();
                } catch (Exception exception) {
                    return CommandResult.fail(exception.toString());
                }
            })));
        ...
        admin.then(RouteNode.literal("help").exec(context -> CommandResult.success()));
            ...
            .then(RouteNode.argument("team", new StringNode())
                .then(RouteNode.argument("balance", new IntNode()).exec(context -> {
                    try {
                        ...
                        context.getSender().sendMessage(prefix + "Der Kontostand von: " + teamName + " wurde auf " + balance + ecoSymbol + " gesetzt");
                        return CommandResult.success();
                    } catch (Exception e) {
                        return CommandResult.fail(e.toString());
                    }
            ...
            .then(RouteNode.argument("team", new StringNode())
                .then(RouteNode.argument("balance", new IntNode()).exec(context -> {
                    try {
                        ...
                        Team team = teamManager.getTeamByName(teamName);
                        String ecoSymbol = ecoManager.getCurrency("s");
                        ecoManager.addMoney(team.id(), balance);
                        context.getSender().sendMessage(prefix + "Dem Kontostand von: " + teamName + " wurden " + balance + ecoSymbol + " hinzugefügt");
                        return CommandResult.success();
                    } catch (Exception e) {
                        return CommandResult.fail(e.toString());
                    }
            ...
            .then(RouteNode.argument("team", new StringNode())
                .then(RouteNode.argument("balance", new IntNode()).exec(context -> {
                    try {
                        ...
                        Team team = teamManager.getTeamByName(teamName);
                        String ecoSymbol = ecoManager.getCurrency("s");
                        ecoManager.removeMoney(team.id(), balance);
                        context.getSender().sendMessage(prefix + "Dem Kontostand von: " + teamName + " wurden " + balance + ecoSymbol + " abgezogen");
                        return CommandResult.success();
                    } catch (Exception e) {
                        return CommandResult.fail(e.toString());
                    }
